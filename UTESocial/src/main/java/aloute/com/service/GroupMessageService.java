...
public class GroupMessageService {
    ...
    @Transactional(readOnly = true)
    public LastMessageInfo getLatestGroupMessageInfo(Integer groupId, Integer currentUserId) {
        // Assuming you have a method like findTopByGroupGroupIdOrderByCreatedAtDesc
        GroupMessage latestMessage = groupMessageRepository.findTopByGroup_GroupIdOrderByCreatedAtDesc(groupId); // Or similar method

        if (latestMessage == null) {
            return new LastMessageInfo("Chưa có tin nhắn", null);
        }

        String preview = generateGroupPreview(latestMessage, currentUserId); // Use helper

        return new LastMessageInfo(preview, latestMessage.getCreatedAt());
    }

    /**
     * ⭐ NEW HELPER: Generates preview text for GroupMessage.
     */
    private String generateGroupPreview(GroupMessage msg, Integer currentUserId) {
        if (msg == null) return "";

        String content = msg.getContent();
        String prefix = "";

        // Add sender name unless it's the current user
        if (msg.getSender() != null) {
            if (msg.getSender().getUserId().equals(currentUserId)) {
                prefix = "Bạn: ";
            } else {
                // Use only the first name for brevity in preview?
                 String senderName = msg.getSender().getFullName().split(" ")[0]; // Get first name
                 prefix = senderName + ": ";
               // prefix = msg.getSender().getFullName() + ": "; // Full name
            }
        }
        ...
        if (content != null && !content.isEmpty()) {
            ...
            // Determine preview based on attachment type
            GroupAttachment firstAtt = msg.getAttachments().iterator().next(); // Assuming GroupAttachment entity
